...
Функция ОтправитьЗапросНаСервер(вопрос) Экспорт
    // Создаем HTTP-запрос
    Запрос = Новый HTTPЗапрос;
    Запрос.УстановитьURL("http://192.168.81.143:8000/query");
    Запрос.УстановитьМетод("POST");
    Запрос.УстановитьЗаголовок("Content-Type", "application/json");

    // Формируем тело запроса
    ТелоЗапроса = Новый Строка;
    ТелоЗапроса = "{""question"": """ + вопрос + """}";
    ...
    // Обрабатываем ответ
    Если Ответ.КодСостояния = 200 Тогда
        Результат = Ответ.ПолучитьТело();
        Сообщить("Ответ: " + Результат);
        Возврат Результат; // Возвращаем ответ от сервера
    Иначе
