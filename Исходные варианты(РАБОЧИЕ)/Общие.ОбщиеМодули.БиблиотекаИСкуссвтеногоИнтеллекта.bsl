...
Процедура ПроверитьИЗаполнитьСправочники() экспорт
	...
	если не выб.Следующий() тогда
		...
		новпровайдер.Записать();
		новпровайдер = справочники.БИИ_Провайдеры.СоздатьЭлемент();
		новпровайдер.Наименование = "local";
		новпровайдер.Записать();
		...
		новмодель = справочники.БИИ_Модели.СоздатьЭлемент();
		новмодель.Наименование = "gemma-3-27b-it-Q4_K_M";
		новмодель.Назначение = "Генерация";
		новмодель.Провайдер = справочники.БИИ_Провайдеры.НайтиПоНаименованию("local");
		новмодель.Записать();
...
Функция Генерация(промт, модель, семплирование=0, инструменты=неопределено, системный_промт=неопределено, полный_результат=ложь, схема_ответа=неопределено, опции=неопределено) экспорт
	если модель.назначение="Генерация" тогда
		...
		// Создаем локальную копию промт для модификации
	локальныйПромт = промт;
	
	// Объединяем системный_промт с промт, если системный_промт определен и промт - строка
	если системный_промт <> неопределено и строка(типзнч(локальныйПромт)) = "Строка" тогда
		локальныйПромт = системный_промт + " " + локальныйПромт;
	конецесли;  
...
// рабочий вариант написания Провайдера под локалку
			Иначеесли модель.провайдер.наименование="local" тогда
				Соединение = Новый HTTPСоединение("192.168.81.23", 1234);
				Заголовки = Новый Соответствие;
				Заголовки.Вставить("Content-Type", "application/json");
				Запрос = Новый HTTPЗапрос("v1/chat/completions", Заголовки);
				мСообщений = новый массив;
...
Функция СоздатьВекторнуюБазу(имя, провайдер) экспорт
	если провайдер.наименование = "OpenAI" тогда
		Соединение = Новый HTTPСоединение("api.openai.com",,,,,,новый ЗащищенноеСоединениеOpenSSL());
		Заголовки = Новый Соответствие;
		Заголовки.Вставить("Content-Type", "application/json");
		...
		если Ответ.КодСостояния = 200 тогда
			...
			данныеответа = ПрочитатьJSON(ЧтениеJSON);
			ЧтениеJSON.Закрыть();
			Сообщить("Создана векторная база OpenAI с ID: " + данныеответа.id);
			возврат данныеответа.id;
		иначе
			Сообщить("Ошибка при создании векторной базы OpenAI: " + Ответ.КодСостояния);
			возврат неопределено;
		конецесли;	
	иначеесли провайдер.наименование = "local" тогда
		Соединение = Новый HTTPСоединение("192.168.81.23", 1234);
		Заголовки = Новый Соответствие;
		Заголовки.Вставить("Content-Type", "application/json");
		Запрос = Новый HTTPЗапрос("v1/vector_stores", Заголовки);
		...
		если Ответ.КодСостояния = 200 тогда
			...
			данныеответа = ПрочитатьJSON(ЧтениеJSON);
			ЧтениеJSON.Закрыть();
			Сообщить("Создана векторная база local с ID: " + данныеответа.id);
			возврат данныеответа.id;
		иначе
			Сообщить("Ошибка при создании векторной базы local: " + Ответ.КодСостояния);
			возврат неопределено;
		конецесли;
	конецесли;	
КонецФункции	
