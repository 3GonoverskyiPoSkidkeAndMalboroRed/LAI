...
Процедура ПроверитьИЗаполнитьСправочники() экспорт
	...
	если не выб.Следующий() тогда
		новыйПровайдер = справочники.БИИ_Провайдеры.СоздатьЭлемент();
		новыйПровайдер.Наименование = "Локальный";
...
если модель.провайдер.наименование="Локальный" тогда
    // Создаем HTTP соединение с локальным сервером
    Соединение = Новый HTTPСоединение("127.0.0.1", 1234);
    Заголовки = Новый Соответствие;
    Заголовки.Вставить("Content-Type", "application/json");
    
    // Формируем запрос к API локального LLM
    Запрос = Новый HTTPЗапрос("/v1/chat/completions", Заголовки);
    
    // Формируем сообщения для запроса
    мСообщений = новый массив;
    
    // Добавляем системный промт, если он есть
    если системный_промт<>неопределено тогда
        стСообщение = новый структура;
        стСообщение.Вставить("role","system");
        стСообщение.Вставить("content",системный_промт);
    ...
    если строка(типзнч(промт))="Строка" тогда
        ...
        мСообщений.Добавить(стСообщение);
    иначе
        // Для обработки мультимодальных запросов, если поддерживаются
        стСообщение = новый структура;
        стСообщение.Вставить("role","user");
        стСообщение.Вставить("content",промт.текст);
        мСообщений.Добавить(стСообщение);
    конецесли;
    ...
    если строка(типзнч(инструменты))="Массив" тогда
        // Код для добавления инструментов аналогичный коду для OpenAI
        минст = новый массив;
        // ... код для обработки инструментов ...
        данные.Вставить("tools", минст);
    конецесли;
    ...
    если Ответ.КодСостояния=200 тогда
        если полный_результат тогда
            ...
            ст.Вставить("Размышления", "");
            ст.Вставить("ВходныеТокены", данныеответа.usage.prompt_tokens);
            ст.Вставить("ВходныеТокеныВКеше", 0);
            ст.Вставить("ВходныеТокеныЗаКешем", данныеответа.usage.prompt_tokens);
            ст.Вставить("ВыходныеТокены", данныеответа.usage.completion_tokens);
            возврат ст;
        иначе    
            возврат данныеответа.choices[0].message.content;
        конецесли;
    иначе
        возврат "Ошибка при обращении к локальному серверу: " + Ответ.КодСостояния;
    конецесли;
конецесли;
