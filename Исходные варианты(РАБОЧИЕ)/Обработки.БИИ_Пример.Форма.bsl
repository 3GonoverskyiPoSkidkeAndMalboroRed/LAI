...
Функция ПроанализироватьJSON(JSONСтрока)
    Попытка
        ...
        // Создаем структуру для хранения результатов анализа
        Анализ = Новый Структура;
        
        // Если данные - это массив (что наиболее вероятно для таблицы)
        Если ТипЗнч(Данные) = Тип("Массив") Тогда
            КоличествоЗаписей = Данные.Количество();
            Анализ.Вставить("КоличествоЗаписей", КоличествоЗаписей);
            
            // Если есть записи, анализируем структуру первой записи для определения полей
            Если КоличествоЗаписей > 0 Тогда
                ...
                Анализ.Вставить("Поля", ИменаПолей);
                
                // Создаем структуру для хранения статистики по полям
                СтатистикаПоПолям = Новый Структура;
                
                // Для каждого поля собираем статистику
                Для Каждого Поле Из ИменаПолей Цикл
                    СтатистикаПоля = Новый Структура;
                    ...
                    Для Каждого Запись Из Данные Цикл
                        Если Запись.Свойство(Поле) Тогда
                            ...
                            Если УникальныеЗначения[Значение] = Неопределено Тогда
                                УникальныеЗначения[Значение] = 1;
                            Иначе
                                УникальныеЗначения[Значение] = УникальныеЗначения[Значение] + 1;
                            КонецЕсли;
                        КонецЕсли;
                    КонецЦикла;
                    
                    СтатистикаПоля.Вставить("КоличествоУникальныхЗначений", УникальныеЗначения.Количество());
                    
                    // Если уникальных значений не слишком много, добавляем их в статистику
                    Если УникальныеЗначения.Количество() <= 10 Тогда
                        ТопЗначения = Новый Соответствие;
                        Для Каждого Элемент Из УникальныеЗначения Цикл
                            ТопЗначения.Вставить(Элемент.Ключ, Элемент.Значение);
                        КонецЦикла;
                        СтатистикаПоля.Вставить("УникальныеЗначения", ТопЗначения);
                    КонецЕсли;
                    
                    СтатистикаПоПолям.Вставить(Поле, СтатистикаПоля);
                КонецЦикла;
                
                Анализ.Вставить("СтатистикаПоПолям", СтатистикаПоПолям);
            КонецЕсли;
        КонецЕсли;
        
        // Формируем строку с результатами анализа
        РезультатАнализа = "Анализ данных из Excel файла:" + Символы.ПС + Символы.ПС;
        
        Если Анализ.Свойство("КоличествоЗаписей") Тогда
            РезультатАнализа = РезультатАнализа + "Общее количество записей: " + Анализ.КоличествоЗаписей + Символы.ПС;
        КонецЕсли;
        
        Если Анализ.Свойство("Поля") Тогда
            РезультатАнализа = РезультатАнализа + "Обнаруженные поля: " + СтрСоединить(Анализ.Поля, ", ") + Символы.ПС + Символы.ПС;
            
            Если Анализ.Свойство("СтатистикаПоПолям") Тогда
                РезультатАнализа = РезультатАнализа + "Статистика по полям:" + Символы.ПС;
                
                Для Каждого Поле Из Анализ.Поля Цикл
                    СтатистикаПоля = Анализ.СтатистикаПоПолям[Поле];
                    РезультатАнализа = РезультатАнализа + "- " + Поле + ": " + СтатистикаПоля.КоличествоУникальныхЗначений + " уникальных значений" + Символы.ПС;
                    
                    Если СтатистикаПоля.Свойство("УникальныеЗначения") Тогда
                        РезультатАнализа = РезультатАнализа + "  Распределение значений:" + Символы.ПС;
                        
                        Для Каждого Элемент Из СтатистикаПоля.УникальныеЗначения Цикл
                            РезультатАнализа = РезультатАнализа + "    " + Элемент.Ключ + ": " + Элемент.Значение + " записей" + Символы.ПС;
                        КонецЦикла;
                    КонецЕсли;
                    
                    РезультатАнализа = РезультатАнализа + Символы.ПС;
                КонецЦикла;
            КонецЕсли;
        КонецЕсли;
        
        Сообщить("[ОТЛАДКА] Анализ JSON успешно выполнен");
