...
Функция ПреобразоватьВДата(строкаДата)
    ...
    // Получаем текущее смещение от UTC
    смещение = Час(ТекущаяДата()) - Час(Дата(0, 0, 0, 0, 0, 0));
    смещениеСтрока = Формат(смещение, "+0#") + ":00"; // Преобразуем в строку формата ±hh:mm

    // Форматируем дату в ISO 8601
    Возврат Формат(дата, "YYYY-MM-DDTHH:MM:SS") + смещениеСтрока;
КонецФункции
