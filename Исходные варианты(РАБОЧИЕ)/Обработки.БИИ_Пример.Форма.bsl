...
&НаКлиенте
Процедура RAGЗапрос(Команда)
	Сообщить("[ОТЛАДКА] Начало процедуры RAGЗапрос");
	стПромт = Промт;
	результат = БиблиотекаИскусственногоИнтеллекта.ОтветНаRAGЗапрос(стПромт);
	Если результат = Неопределено Тогда
		Сообщить("[ОШИБКА] Не удалось получить ответ от RAGЗапроса");
		Возврат;
	КонецЕсли;
	Ответ = результат; // Сохраняем ответ в реквизит формы Ответ
	Сообщить("[ОТЛАДКА] Получен ответ из RAGЗапроса");
КонецПроцедуры
