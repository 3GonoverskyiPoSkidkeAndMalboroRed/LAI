...
Процедура Вызов(имя, параметры)
	...
	Если имя = "GetGoodsBalance" Тогда
		// Проверяем, пустые ли параметры и есть ли текст в промте
		Если (параметры = "" Или параметры = "{}" Или параметры = Неопределено) И ЗначениеЗаполнено(Промт) Тогда
			// Создаем структуру с параметром GoodsName из текста промта
			ЗаписьJSON = Новый ЗаписьJSON;
			ЗаписьJSON.УстановитьСтроку();
			
			СтруктураПараметров = Новый Структура;
			СтруктураПараметров.Вставить("GoodsName", Промт);
			
			ЗаписатьJSON(ЗаписьJSON, СтруктураПараметров);
			параметрыJSON = ЗаписьJSON.Закрыть();
			
			Сообщить("[ОТЛАДКА] Сформированы параметры из промта: " + параметрыJSON);
			Ответ = Ответ + GetGoodsBalance(параметрыJSON) + символы.ПС;
		Иначе
